...
<div class="tab-content" id="div_tab1"<?php echo $tab == 1 ? ' style="display: block;"' : ''; ?>>
    <table class="tab-content-table">
        ...
        <td>Шаблон заголовка<span id="span-title-vk" <?php if ($row['type'] != 'vk') { ?>style="display:none;"<?php } ?>> при отсутствии основного текста</span></td><td><input type="text" name="row[title]" value="<?php echo htmlentities($row['title'], ENT_COMPAT, 'UTF-8'); ?>" size="100" /></td>
        </tr>
        <tr id="tr-title-words" <?php if ($row['type'] != 'vk') echo 'style="display:none;"'; ?>>
        <td>Кол-во слов в заголовке</td><td><input type="text" name="params[title_words_count]" value="<?php echo htmlentities($row['params']['title_words_count'], ENT_COMPAT, 'UTF-8'); ?>" size="5" style="text-align: center;" /></td>
        </tr>
...
<div class="tab-content" id="div_tab3"<?php echo $tab == 3 ? ' style="display: block;"' : ''; ?>>
    <table class="tab-content-table">
        ...
        <tr>
            <td>Сохранять картинки на сервере</td><td><?php echo WPGHelper::yesNoRadioList('params[image_save]', $row['params']['image_save']); ?></td>
        </tr>
        <tr>
    ...
    <table class="tab-content-table">
      ...
      <tr>
        ...
        <tr<?php echo ($selected_provider != 0) ? ' style="display: none;"' : ''; ?> id="yandex-api-key">
          ...
          <td>
            <input type="text" name="params[yandex_api_key]" value="<?php echo WPGTools::esc($row['params']['yandex_api_key']); ?>" size="80" />
          </td>
        </tr>
    </table>
  </fieldset>
    ...
    <table class="tab-content-table">
      ...
      <tr>
        ...
        <tr<?php echo ($selected_provider != 0) ? ' style="display: none;"' : ''; ?> id="yandex-api-key2">
          <td>API-ключ для Яндекс.Перевода</td>
          <td>
            <input type="text" name="params[yandex_api_key2]" value="<?php echo WPGTools::esc($row['params']['yandex_api_key2']); ?>" size="80" />
          </td>
        </tr>
    </table>
  </fieldset>
    ...
    <table class="tab-content-table">
        ...
        <div style="overflow: auto; height: 400px;">
            ...
            <table class="truser">
                ...
                <tr align="center">
                <td><?php echo ($i+1); ?></td>
                <td><?php echo WPGHelper::selectList("params[usrepl][$i][type]", array('0' => 'выключен', 'index' => 'индексная страница (rss-контент)', 'page' => 'страница контента', 'intro' => 'анонс', 'text' => 'полный текст', 'title' => 'заголовок'), $row['params']['usrepl'][$i]['type'], 1); ?></td>
                <td><input size="60" type="text" name="params[usrepl][<?php echo $i; ?>][name]" value="<?php echo $row['params']['usrepl'][$i]['name']; ?>" /></td>
                ...
                <td><input size="60" type="text" name="params[usrepl][<?php echo $i; ?>][replace]" value="<?php echo htmlspecialchars($row['params']['usrepl'][$i]['replace']); ?>" /></td>
        ...
        <tr>
          <td valign="top">Текст</td>
            <td>
                ...
                <tr valign="top">
                <td><textarea name="params[template_full_text]" style="width:410px" rows="12"><?php echo $row['params']['template_full_text']; ?></textarea></td>
...
<div class="tab-content" id="div_tab8"<?php echo $tab == 8 ? ' style="display: block;"' : ''; ?>>
    <table class="tab-content-table">
        <tr>
        <td width="225">Для запросов использовать метод</td><td><?php echo WPGHelper::selectList('params[requestMethod]', array('по умолчанию', 'CURL', 'file_get_contents'), $row['params']['requestMethod'], 1); ?></td>
        </tr>
    </table>
    <fieldset>
